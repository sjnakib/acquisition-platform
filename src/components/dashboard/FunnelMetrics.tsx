          ...
          return (
            <div key={stage.label} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-7 bg-slate-100 rounded relative overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-blue-500 to-green-500 rounded transition-all"
                  style={{ width: `${width}%` }}
                />
              </div>
              <span className="text-xs font-medium text-slate-700 w-16">{stage.count}</span>
            </div>
          )
        })}
      </div>
