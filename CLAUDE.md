...
- **`src/lib/validations/`** — 10 Zod schemas: `auth`, `call`, `campaign`, `contact`, `deal`, `import`, `invitation`, `portfolio`, `project`, `template`. API routes import from here for request body validation.
...
- CSP headers in `next.config.ts` — adding external APIs/scripts/iframes requires updating CSP. Also sets `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`.
  - script-src: `'self'`, `'unsafe-inline'`, `'unsafe-eval'`, `https://challenges.cloudflare.com`, `https://*.googleapis.com`
...
| Doc | Content |
|---|---|
| `PLAN.md` | Original build blueprint — schema details, Supabase API patterns. Some details diverged in implementation; verify against actual migrations (51 exist, PLAN.md describes 17). |
