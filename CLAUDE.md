...
@AGENTS.md — Next.js 16 specifics + migration timeline + DB quick reference. Read that for Next.js 16 gotchas and schema history.
...
- **Components** in `src/components/` by domain: `ui/`, `shared/`, `auth/`, `admin/`, `dashboard/`, `deals/`, `client/`, `import/`, `campaigns/`, `portfolios/`, `projects/`
...
- **Other lib files:** `batch-delete.ts`, `directory-traversal.ts`, `navigation.ts` (sidebar nav item definitions), `stage-machine.ts`, `rate-limit.ts` (Upstash Redis sliding-window: login 5/5min, email 100/1day, password-reset 3/1min), `brand.ts`, `page-headings.ts`, `turnstile.ts`, `utils.ts`. `src/lib/types/grid.ts` for grid-related types.
...
- **Google integration:** `src/lib/google/gmail.ts`, `drive.ts`, `oauth.ts`, `people.ts`, `auth-cache.ts` (in-memory OAuth2 client cache, 5min TTL) — Gmail API (push notifications via `gmail.users.watch()`, Pub/Sub webhook at `/api/emails/webhook`, snoozed threads via migration 0035), Drive API (linked deal rooms — folder provisioning per deal via `drive_folders` table, migration 0041; `DriveFileManager` component for browsing/uploading), People API (contact lookups), OAuth (offline refresh tokens stored in `google_connections` keyed by Google email, not user). Multi-project: each project can connect to any Google account via `google_connection_id` FK; OAuth state encodes `projectId` for per-project connect flow. System Gmail (migration 0051): `google_connections.connection_type` distinguishes `project` vs `system` connections for transactional emails (invitations). API routes: `auth/google/callback`, `auth/google/refresh-watch`, `projects/[id]/google/disconnect`, `deals/[id]/drive/files`. Email templates: `email_templates` table (migration 0032) — project-scoped custom templates for outreach emails; `custom` enum value added in 0033. Email attachments stored via migration 0037. `src/lib/email/send.ts` sends via React Email (render) + Gmail system connection (no Resend); templates in `src/lib/email/templates/` (declination, invitation, outreach, password-reset, thank-you).
- **Google OAuth reconnect (`GOOGLE_AUTH_RECONNECT.md`):** `GoogleAuthError` class (code: `invalid_grant` | `not_connected`) in `oauth.ts`. Static `isInvalidGrant()` detects raw Google API errors. `callWithConnection(connectionId, fn)` wrapper — all gmail.ts/drive.ts/people.ts functions pass through it; catches `invalid_grant` → `invalidateConnection()` (evicts cache + nullifies tokens in `google_connections`) → throws typed `GoogleAuthError`. Three-state connection model via `useGoogleConnection(projectId?)` returning `{ status: 'connected' | 'expired' | 'disconnected' }`. `GET /api/projects/[id]` and `GET /api/admin/system-email` return `token_valid: boolean` for connection health without Google API call. `GoogleReconnectDialog` (shared Radix Dialog) for reconnect UX. API routes catch `instanceof GoogleAuthError` → 401 `{ error: 'google_auth_expired' }`. Frontend detects `google_auth_expired` → opens dialog / shows expired UI. OAuth callback preserves existing `refresh_token` on re-auth to prevent permanent breakage. `returnTo` parameter flows through OAuth state → callback redirect back to current page.
...
- **Shared components:** `DataGrid` (virtualized Excel-like table), `ProjectContext` (project state provider — wraps children with current project), `Sidebar`, `Breadcrumb`, `PageHeader`, `InlineDropdownEditor` (inline select for DataGrid enum columns: stage, score, portfolio, response classification), `EmailComposer` (shared email composition, used by both internal and client), `EmailThreadList`, `EmailMessagePanel`, `FileDropZone`, `UploadPanel`, `GoogleReconnectDialog`, `PageTransition`, `ReactQueryProvider`, `PaginationControls`, `LoadingSpinner`, `EmptyState`, `BrandLogo`. Notable domain components: `DealDetailView` (`components/deals/`), `CampaignCard` (`components/campaigns/`).
...
- **Brand:** `src/lib/brand.ts` (`BRAND.name = 'Acquire'`, `BRAND.tagline = 'Acquisition Platform'`, `BRAND.description = 'Multifamily Property Acquisition Platform'`). **Page headings:** `src/lib/page-headings.ts`.
...
- `src/lib/email/send.ts` uses React Email (render) + Gmail system connection for transactional emails; no Resend dependency
